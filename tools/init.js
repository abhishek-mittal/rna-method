...
async function main() {
  ...
  // ── Phase 3.5: MCP / Tool Discovery ──────────────────────────────────────

  console.log('');
  console.log(c('bold', '  ─ Scanning for MCP servers & tools ──────────────'));

  let toolDiscovery = null;
  let toolManifest  = null;
  try {
    let discoverMod;
    if (IS_EMBEDDED) {
      discoverMod = require(path.join(REPO_ROOT, 'tools', 'discover-tools'));
    } else {
      const discoverCode = await getFile('tools/discover-tools.js');
      const tmpDiscPath  = path.join(tmpDir, 'discover-tools.js');
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(tmpDiscPath, discoverCode, 'utf-8');
      discoverMod = require(tmpDiscPath);
    }

    toolDiscovery = discoverMod.discover(platform, outputDir);

    if (toolDiscovery.serverCount === 0) {
      console.log(c('gray', '  (no MCP servers detected)'));
    } else {
      console.log(`  Found ${c('green', String(toolDiscovery.serverCount))} MCP server(s):`);
      for (const [key, srv] of Object.entries(toolDiscovery.servers)) {
        const tag = srv.known ? c('green', '✓ known') : c('yellow', '? unknown');
        console.log(`    ${tag}  ${srv.name || key}`);
      }

      // Compute per-agent MCP tool assignments
      const agentMcpTools = discoverMod.computeAgentMcpTools(toolDiscovery, selectedAgents);

      // Inject mcpTools into schema agents
      for (const agent of schema.agents) {
        if (agentMcpTools[agent.id] && agentMcpTools[agent.id].length > 0) {
          agent.mcpTools = [...new Set([...(agent.mcpTools || []), ...agentMcpTools[agent.id]])];
        }
      }

      // Build manifest for .rna/tools-manifest.json
      toolManifest = discoverMod.buildManifest(toolDiscovery, agentMcpTools);

      const assignedCount = Object.keys(agentMcpTools).length;
      if (assignedCount > 0) {
        console.log(`  Assigned MCP tools to ${c('green', String(assignedCount))} agent(s)`);
      }
    }
  } catch (e) {
    console.log(c('yellow', `  ⚠ Tool discovery skipped: ${e.message}`));
  }
  ...
  // Write tools manifest if discovery found servers
  if (toolManifest) {
    const manifestPath = path.join(rnaConfigDir, 'tools-manifest.json');
    writef(manifestPath, JSON.stringify(toolManifest, null, 2) + '\n');
    console.log('  ✓ .rna/tools-manifest.json');
  }
