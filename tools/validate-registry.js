...
try {
  receptors = JSON.parse(fs.readFileSync(RECEPTORS_PATH, 'utf8'))
} catch (e) {
  console.error(`✗ FATAL: Cannot read receptors.json at ${RECEPTORS_PATH}`)
...
try {
  ...
  // agent-context.json may not exist in minimal setups — treat as warning, not fatal
  if (!JSON_MODE) {
    console.warn(`⚠ agent-context.json not found at ${CONTEXT_PATH}`)
...
// ─── Check 6: No orphaned checkpoint pointers in agent-context.json ──────
...
for (const p of results.passed) {
  console.log(`${GREEN}✓${RESET} [${p.check}] ${p.detail}`)
}
for (const w of results.warnings) {
  console.log(`${YELLOW}⚠${RESET} [${w.check}] ${w.detail}`)
}
for (const f of results.failed) {
  console.log(`${RED}✗${RESET} [${f.check}] ${f.detail}`)
}
...
if (results.suggestions.length > 0) {
  ...
  for (const s of results.suggestions) {
    console.log(`  [${s.check}] ${s.suggestion}`)
  }
}
